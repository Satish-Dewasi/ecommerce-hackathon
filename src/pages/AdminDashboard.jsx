...
import {
  ...
  Trash2, Ban, RefreshCw, ChevronDown, X,
  Search, Play, CheckCircle, AlertCircle, Crown
} from "lucide-react";
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    <div className="relative bg-background border border-foreground/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
      ...
      <div className="flex gap-3">
        <button onClick={onCancel} className="flex-1 border border-foreground/20 rounded-md py-2.5 text-sm font-medium hover:bg-secondary/60 transition-colors">
          Cancel
        </button>
        <button onClick={onConfirm} className="flex-1 bg-red-600 text-white rounded-md py-2.5 text-sm font-bold hover:bg-red-700 transition-colors">
          Confirm
        </button>
      </div>
    </div>
...
const AdminDashboard = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await api.get("/v1/user/admin/users");
      setUsers(data.users || []);
    } catch (err) {
  ...
  useEffect(() => {
    if (activeTab === "users" || activeTab === "sellers" || activeTab === "overview") {
      fetchUsers();
    }
  ...
  const handleBan = (u) => {
    if (u.isBanned) {
      ...
      setConfirm({
        ...
        action: async () => {
          try {
            const data = await api.put(`/v1/user/admin/users/${u._id}/ban`, { isBanned: false });
  ...
  const submitBan = async () => {
    if (!banTarget) return;
    try {
      const data = await api.put(`/v1/user/admin/users/${banTarget._id}/ban`, {
        isBanned: true,
        banReason: banReason || "Policy violation",
      });
      setUsers((prev) => prev.map((x) => x._id === banTarget._id ? data.user : x));
    } catch (err) { setError(err.message); }
    setBanTarget(null);
    setBanReason("");
  };
    ...
    <div className="min-h-screen bg-background text-foreground flex">
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6 md:p-8 max-w-6xl">
          ...
          {activeTab === "overview" && (
            <div>
              <h1 className="text-3xl font-black uppercase mb-1" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                Overview
              </h1>
              <p className="text-sm text-muted-foreground mb-8">Welcome back, {user?.name}</p>

              {loading ? (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="border border-foreground/10 rounded-2xl p-6 animate-pulse">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                  <StatCard label="Total Users"    value={users.length}      sub="All accounts"        />
                  <StatCard label="Customers"      value={allUsers.length}   sub="Role: customer"      />
                  <StatCard label="Sellers"        value={sellers.length}    sub="Active storefronts"  />
                  <StatCard label="Banned"         value={banned.length}     sub="Restricted accounts" accent="border-red-200 dark:border-red-800" />
                </div>
              )}

              {/* Recent users */}
              <div>
                <h2 className="text-sm font-black uppercase tracking-widest mb-4">Recently Joined</h2>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-foreground/10 bg-secondary/20">
                        ...
                        <th className="text-left px-4 py-3 text-xs font-bold uppercase tracking-wider text-muted-foreground hidden md:table-cell">Joined</th>
                      </tr>
                    </thead>
                    <tbody>
                      {users.slice(0, 5).map((u) => (
                        <tr key={u._id} className="border-b border-foreground/5 hover:bg-secondary/20 transition-colors">
          ...
          {(activeTab === "users" || activeTab === "sellers") && (
            <div>
              <div className="flex items-start justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {displayList.length} {activeTab === "sellers" ? "seller" : "user"}{displayList.length !== 1 ? "s" : ""}
                    {search ? ` matching "${search}"` : ""}
                  </p>
                </div>
                <button onClick={fetchUsers} className="p-2 hover:bg-secondary/60 rounded-lg transition-colors" title="Refresh">
              ...
              <p className="text-xs text-muted-foreground mt-4">
                ⚠️ Role changes are reflected locally. Ask your backend teammate to add{" "}
                <code className="bg-secondary/60 px-1 rounded">PUT /api/v1/user/admin/users/:id/role</code>{" "}
                to persist them.
              </p>
            </div>
          )}
      ...
      {banTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-background border border-foreground/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <h3 className="text-sm font-black uppercase tracking-wider mb-1">Ban User</h3>
            ...
            />
            <div className="flex gap-3">
              <button onClick={() => setBanTarget(null)} className="flex-1 border border-foreground/20 rounded-md py-2.5 text-sm font-medium hover:bg-secondary/60 transition-colors">
                Cancel
              </button>
              <button onClick={submitBan} className="flex-1 bg-red-600 text-white rounded-md py-2.5 text-sm font-bold hover:bg-red-700 transition-colors">
                Ban User
              </button>
            </div>
          </div>
