...
import { ArrowLeft, ChevronDown, Check, CreditCard, Truck, Package } from "lucide-react";
...
const INPUT = "w-full border border-foreground/15 rounded-md px-4 py-3.5 text-sm bg-transparent focus:outline-none focus:border-foreground/40 transition-colors placeholder:text-muted-foreground";
...
const Checkout = () => {
  ...
  const [info, setInfo] = useState({
    ...
    address2:   user?.addresses?.[0]?.line2 || "",
  ...
  const canProceed = () => {
    if (step === 0) {
      return (
        info.email && info.firstName && info.lastName &&
  ...
  const handlePlaceOrder = async () => {
    setLoading(true);
    // Simulate a brief delay (replace with real order API when backend has it)
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="flex items-center justify-between px-6 md:px-10 py-5 border-b border-foreground/10">
        <button onClick={() => step > 0 ? setStep(s => s - 1) : navigate(-1)} className="flex items-center gap-2 text-sm hover:opacity-70 transition-opacity">
          ...
          {STEPS.map((s, i) => (
            <div key={s.label} className="flex items-center">
              ...
              >
                ...
                <span className="text-sm font-bold uppercase tracking-wider hidden sm:block">{s.label}</span>
              </button>
              {i < STEPS.length - 1 && (
                <div className={`w-8 md:w-16 h-px mx-2 md:mx-4 ${i < step ? "bg-foreground" : "bg-foreground/20"}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="flex-1 max-w-lg">
            ...
            {step === 0 && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-sm font-black uppercase tracking-wider mb-4">Contact</h2>
                  <div className="space-y-3">
                    <input type="email"   placeholder="Email *"   value={info.email}  onChange={setI("email")} className={INPUT} />
                    <input type="tel"     placeholder="Phone"     value={info.phone}  onChange={setI("phone")} className={INPUT} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-3">
                      <input placeholder="First Name *" value={info.firstName} onChange={setI("firstName")} className={INPUT} />
                      <input placeholder="Last Name *"  value={info.lastName}  onChange={setI("lastName")}  className={INPUT} />
                    </div>
                    <input placeholder="Address Line 1 *" value={info.address}  onChange={setI("address")}  className={INPUT} />
                    <input placeholder="Address Line 2"   value={info.address2} onChange={setI("address2")} className={INPUT} />
                    <div className="flex gap-3">
                      <input placeholder="City *"   value={info.city}   onChange={setI("city")}   className={INPUT} />
                      <input placeholder="State *"  value={info.state}  onChange={setI("state")}  className={INPUT} />
                    </div>
                    <div className="flex gap-3">
                      <input placeholder="Pincode *" value={info.pincode} onChange={setI("pincode")} className={INPUT} />
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                {[
                  { id: "standard", label: "Standard Delivery",  eta: "5–7 business days", price: 99  },
                  { id: "express",  label: "Express Delivery",   eta: "2–3 business days", price: 199 },
                  { id: "next_day", label: "Next Day Delivery",  eta: "1 business day",    price: 299 },
                ].map((m) => (
                  <label
                    key={m.id}
                ...
                <div className="border border-foreground/10 rounded-xl p-4 mt-6">
                  <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Delivering to</p>
                  <p className="text-sm font-medium">{info.firstName} {info.lastName}</p>
                  <p className="text-sm text-muted-foreground">{info.address}, {info.city}, {info.state} {info.pincode}</p>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-5">
                <h2 className="text-sm font-black uppercase tracking-wider mb-4">Payment Method</h2>
                ...
                {payment.method === "card" && (
                  <div className="space-y-3">
                    <input placeholder="Cardholder Name"  value={payment.name}   onChange={setP("name")}   className={INPUT} />
                    <input placeholder="Card Number"      value={payment.card}   onChange={setP("card")}   maxLength={19} className={INPUT} />
                    <div className="flex gap-3">
                      <input placeholder="MM / YY"   value={payment.expiry} onChange={setP("expiry")} className={INPUT} />
                      <input placeholder="CVV"        value={payment.cvv}    onChange={setP("cvv")}    maxLength={3} className={INPUT} type="password" />
                    </div>
                ...
                {payment.method === "upi" && (
                  <div>
                    <input placeholder="Enter UPI ID (e.g. name@upi)" value={payment.upi} onChange={setP("upi")} className={INPUT} />
                ...
                {payment.method === "cod" && (
                  <div className="border border-foreground/10 rounded-xl p-5 text-sm text-muted-foreground">
                    <p className="font-bold text-foreground mb-1">Cash on Delivery</p>
                    <p>Pay in cash when your order arrives. Additional ₹30 COD fee may apply.</p>
                  </div>
                )}
                ...
                >
                  {loading ? "Placing Order…" : <>Place Order <Check className="w-4 h-4" /></>}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="lg:w-[360px] shrink-0">
            <div className="border border-foreground/10 rounded-2xl p-6 sticky top-6">
              <h2 className="text-sm font-black uppercase tracking-wider mb-5">Your Order</h2>

              <div className="space-y-4 mb-5">
                {items.map((item) => (
                  <div key={item.id + (item.variantSku || "")} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.variantSku && <p className="text-xs text-muted-foreground">{item.variantSku}</p>}
