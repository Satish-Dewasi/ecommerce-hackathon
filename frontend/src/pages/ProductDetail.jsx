...
const ProductDetail = () => {
  ...
  const allImages = product?.images?.[selectedColor] || [];
  ...
  const selectVariant = (color, size) => {
    const match =
      ...
      product.variants?.find((v) => v.color === (color || selectedColor)) ||
      product.variants?.find((v) => v.size === (size || selectedSize));
  ...
  // ── Price to display ──────────────────────────────────────────────────────
  const displayPrice =
    selectedVariant?.price ?? product?.salePrice ?? product?.basePrice ?? 0;
  ...
  if (error)
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/products" className="text-sm font-bold underline">
          ← Back to Products
        </Link>
      </div>
    );
                ...
                {allImages.map((img, i) => (
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        e.target.src =
                          "https://placehold.co/80x100/f5f5f5/999?text=·";
                      }}
                    />
                  </button>
                ))}
            ...
            <div className="flex-1 rounded-2xl overflow-hidden bg-secondary/30 relative">
              {allImages[selectedImage] ? (
                <img
                  ...
                  onError={(e) => {
                    e.target.src =
                      "https://placehold.co/600x700/f5f5f5/999?text=No+Image";
                  }}
                />
              ) : (
                    ...
                    {uniqueSizes.map((s) => {
                      ...
                      return (
                        <button
                          key={s}
                          onClick={() =>
                            !outOfStock && selectVariant(selectedColor, s)
                          }
                          disabled={outOfStock}
